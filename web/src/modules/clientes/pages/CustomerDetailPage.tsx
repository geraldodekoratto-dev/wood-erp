  ...
  if (error || !customer) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        <Users size={36} style={{ color: '#1e3a5f' }} />
        <p style={{ color: '#475569' }}>{error || 'Cliente não encontrado.'}</p>
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-xl font-bold text-white">{customer.name}</span>
      ...
      <div className="rounded-xl p-5 mb-4" style={{ background: '#0f2040', border: '1px solid rgba(255,255,255,0.07)' }}>
        <p className="text-xs font-semibold uppercase tracking-wider mb-4" style={{ color: '#475569' }}>Contato</p>
      ...
      {(customer.street || customer.city || customer.zip_code) && (
        <div className="rounded-xl p-5 mb-4" style={{ background: '#0f2040', border: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#475569' }}>Endereço</p>
          </div>
          <p className="text-sm" style={{ color: '#94a3b8' }}>{fullAddress(customer)}</p>
        </div>
      )}
      ...
      {customer.notes && (
        <div className="rounded-xl p-5 mb-4" style={{ background: '#0f2040', border: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#475569' }}>Observações</p>
          </div>
          <p className="text-sm leading-relaxed" style={{ color: '#94a3b8' }}>{customer.notes}</p>
        </div>
      )}

      {/* Timestamps */}
      <div className="text-xs mt-6" style={{ color: '#334155' }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-sm rounded-2xl p-6"
            ...
            <p className="text-sm text-center mb-6" style={{ color: '#64748b' }}>
              <span className="font-semibold text-white">{customer.name}</span> será marcado como{' '}
...
function InfoCard({ icon, label, value, mono }: { icon: React.ReactNode; label: string; value: string; mono?: boolean }) {
  return (
    <div className="rounded-xl p-4" style={{ background: '#0f2040', border: '1px solid rgba(255,255,255,0.07)' }}>
      ...
      <p className={`text-sm font-medium ${mono ? 'font-mono' : ''}`} style={{ color: '#e2e8f0' }}>{value}</p>
    </div>
  )
}
